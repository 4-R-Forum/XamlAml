function Global:Load-Excel {
    ...
    For ($s=1; $s -le $sht_ct; $s++) # for each sheet
    {
        ...
        if (-not (($this_sht.Name).StartsWith($ignore_pfx))) # ignore sheets named to ignore
        {
            ...
            # step 5.4iterate across sheets, and their rows to load data
            for ($r = 3 ; $r -le $row_ct; $r++) # for each row
            {   
                ...
                {
                    $this_col_name = $this_sht.Cells[2, $c].Value
                    if (-not ($this_col_name.StartsWith($ignore_pfx))) # ignore columns starting with _
                    {
                        ...
                        if (-not ($this_col_name -eq "physical_file")) # ignore column "physical_file", reserved for loading Files
                        {
                            ...
                            if (-not ([string]::IsNullOrEmpty($this_cell_value))) # skip empty columns
                            {
                                if ($this_col_name -match $regex1) # this is a property of type Item
                                {           
                                ...
                                {
                                    $this_item.setProperty($this_col_name , $this_Cell_value) # property represented by a string
                                    # ///TODO convert dates to neutral
                                } 
                            }
                         }
                    }
                ...
                {
                    $res = $this_item.apply()
                    if ($res.isError()) {throw $res.getErrorString()}
                }
